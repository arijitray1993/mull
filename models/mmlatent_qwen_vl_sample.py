...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if self.config.stage == 'stage2' and not generate_mode and latent_hidden_states is None:
            ...
            kv_cache = None
            all_log_probs = []
            all_pause_embeddings = torch.zeros(inputs_embeds.shape[0], max_n_latents, inputs_embeds.shape[-1])
            for latent_i in range(max_n_latents):
                ...
                for idx_pair in filling_indices:
                    ...
                    if self.config.use_latent_projection:
                        ...
                        prev_embedding_means = self.latent_sample_projection(prev_embedding)
                        ...
                        if self.config.latent_sample_temperature!=0:
                            pause_means = prev_embedding_means.float()
                            ...
                            pause_embeddings = prev_embedding_means
                    else:
                    ...
                    all_pause_embeddings[batch_idx, latent_i, :] = pause_embeddings
            ...
            if len(all_log_probs) > 0:
                all_log_probs = torch.stack(all_log_probs).to(inputs_embeds.device)
                # this all_log_probs is batch*latent with all_latents_batch0, all_latents_batch1, ... all concatenated together. 
                # this means that the first batch_size number of log_probs are for the first latent, the next batch_size number of log_probs are for the second latent, etc. 
                # let's reshape this to be batch*latent_nums
                batch_size = input_ids.shape[0]
                all_log_probs = all_log_probs.view(-1, batch_size).T
            else:
                all_log_probs = None
